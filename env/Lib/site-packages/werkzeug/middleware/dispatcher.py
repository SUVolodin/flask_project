...
This example dispatches to an API webapp under "/api", an admin webapp
under "/admin", and an webapp that serves frontend files for all other
requests::

    webapp = DispatcherMiddleware(serve_frontend, {
        '/api': api_app,
        '/admin': admin_app,
    })
